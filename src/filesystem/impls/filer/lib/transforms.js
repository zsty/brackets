...
define(function(require, exports, module) {
    ...
    function _applyTransform(path, data) {
      transform.transform(path, data, function(err, transformed) {
        ...
        _fs.writeFile(transformedPath, transformed, function(err) {
          if (err) {
            console.error(
              "[Bramble Error] unable to write transformed file",
              transformedPath,
              err
            );
            return callback(err);
          ...
          Handlers.handleFile(transformedPath, data, function(err) {
            if (err) {
              console.error(
                "[Bramble Error] unable to rewrite URL for transformed file",
                transformedPath,
                err
              );
              return callback(err);
  ...
  function _setupMarkdownTransform() {
    ...
    var markdownExts = LanguageManager.getLanguage(
      "markdown"
    ).getFileExtensions();
    markdownExts.forEach(function(ext) {
      _register(
        new Transform({ srcExt: ext, destExt: "html" }, markdownToHTML)
      );
    });
  }
  ...
  function _setupLessTransform() {
    function lessToCSS(path, less, callback) {
      ExtensionUtils.parseLessCode(less, path)
        ...
        .fail(function() {
          callback(
            new Error("[Bramble transform] unable to parse less file " + path)
          );
        });
    }
