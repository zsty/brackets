...
define(function(require, exports, module) {
  ...
  var extensionInfo = JSON.parse(
    require("text!extensions/bramble-extensions.json")
  );
  ...
  function _processDefaults(disableExtensions) {
    disableExtensions = disableExtensions
      ? disableExtensions.trim().split(/\s*,\s*/)
      : [];
    ...
    extensionInfo.forEach(function(info) {
      ...
      if (
        disableExtensions.indexOf(extBasename) > -1 ||
        disableExtensions.indexOf(extPath) > -1
      ) {
        console.log(
          "[Bramble] Skipping loading of extension " +
            extBasename +
            " at " +
            extPath
        );
