...
define(function(require, exports, module) {
  ...
  var LiveDevMultiBrowser = brackets.getModule(
    "LiveDevelopment/LiveDevMultiBrowser"
  );
  var LiveDevProtocol = brackets.getModule(
    "LiveDevelopment/MultiBrowserImpl/protocol/LiveDevProtocol"
  );
  ...
  function ensureLiveDoc() {
    ...
    FileSystem.resolve(liveDocPath, function(err, file) {
      if (err) {
        console.log(
          "[Bramble Error] unable to open path in editor",
          liveDocPath,
          err
        );
        return;
  ...
  function getHighlightInfo(msg) {
    var match = msg.match(
      /^bramble-highlight-lines\:([0-9]+),([0-9]+),([0-9]+),([0-9]+),?([0-9]+)?/
    );
  ...
  function highlightLine(msg) {
    ...
    _prevLineMarker = codeMirror.markText(from, to, {
      className: "bramble-inspector-highlight"
    });
  }
  ...
  function getRemoteScript(filename) {
    ...
    return "<script>window.___brambleFilename = '" +
      filename +
      "';</script>\n" +
      "<script>\n" +
      MouseManagerRemote +
