...
define(function(require, exports, module) {
  ...
  var HTMLRewriter = brackets.getModule(
  ...
  function _brambleCommand(command, args, callback) {
    ...
    switch (command) {
      ...
      case "BRAMBLE_ADD_CODE_SNIPPET":
        skipCallback = true;
        CommandManager.execute("bramble.addCodeSnippet", args[0]).always(
          callback
        );
        break;
      default:
    ...
    switch (remoteRequest.commandCategory) {
      case "brackets":
        _bracketsCommand(
          ...
          args,
          _remoteCallbackFn(remoteRequest.callback)
        );
        break;
      case "bramble":
        _brambleCommand(
      ...
      default:
        console.error(
          "[Bramble] unknown remote command request:",
          remoteRequest
        );
        break;
    }
