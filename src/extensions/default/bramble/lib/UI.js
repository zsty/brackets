...
define(function(require, exports, module) {
  ...
  function removeTitleBar() {
    ...
    $("#first-pane .pane-content, .cm-s-light-theme").addClass(
      "first-pane-height"
    );
  }
  ...
  function toggleMobileViewButton() {
    var mobileView = Mustache.render(
      "<div><a id='mobileViewButton' href=#></a></div>",
      Strings
    );
  ...
  function addLivePreviewButton(callback) {
    var livePreview = Mustache.render(
      "<div><a id='liveDevButton' href=#></a></div>",
      Strings
    );
    StatusBar.addIndicator(
      "liveDevButtonBox",
      ...
      true,
      "",
      "Click to open preview in separate window",
      "mobileViewButtonBox"
    );
    ...
    Compatibility.supportsIFrameHTMLBlobURL(function(err, isCompatible) {
      ...
      if (!isCompatible && document.all) {
        ...
        console.log(
          "[Brackets IFrame-Browser] Detachable preview disabled due to incompatibility with current browser (you are possibly running IE 10 or below)"
        );
      }
