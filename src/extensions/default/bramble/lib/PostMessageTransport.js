...
define(function(require, exports, module) {
  "use strict";
  var _iframeRef, connId = 1;

  var Launcher = require("lib/launcher"),
    Browser = require("lib/iframe-browser");

  var EventDispatcher = brackets.getModule("utils/EventDispatcher"),
    LiveDevMultiBrowser = brackets.getModule(
  ...
  function resolvePaths(message) {
    ...
    var linkRegex = new RegExp(
      '(\\\\?\\"?)(href|src|url|value)(\\\\?\\"?\\s?:?\\s?\\(?\\\\?\\"?)([^\\\\"\\),]+)(\\\\?\\"?)',
      "gm"
    );
    var resolvedMessage = message.replace(
      linkRegex,
      function(match, quote1, attr, seperator, value, quote2) {
        var path = value.charAt(0) === "/"
          ? value
          : Path.join(currentDir, value);
