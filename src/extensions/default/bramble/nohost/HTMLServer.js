...
define(function(require, exports, module) {
  "use strict";
  var BaseServer = brackets.getModule(
    "LiveDevelopment/Servers/BaseServer"
  ).BaseServer,
    LiveDevelopmentUtils = brackets.getModule(
      "LiveDevelopment/LiveDevelopmentUtils"
    ),
    ...
    Path = Filer.Path,
    HTMLRewriter = brackets.getModule(
  ...
  var fs = Filer.fs(), _shouldUseBlobURL;
  ...
  HTMLServer.prototype.serveLiveDocForPath = function(path, callback) {
    ...
    function serveHTML(path, html, server, callback) {
      HTMLRewriter.rewrite(path, html, server, function(err, html) {
        ...
        callback(
          null,
          _shouldUseBlobURL
            ? BlobUtils.createURL(path, html, "text/html")
            : html
        );
      });
    }
    ...
    function serve(body) {
      if (_isHTML(path)) {
        ...
        callback(
          new Error(
            "[Brackets HTMLServer] expected .html or .css live doc type:" + path
          )
        );
      }
    }
