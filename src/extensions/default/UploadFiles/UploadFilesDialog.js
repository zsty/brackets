...
define(function(require, exports, module) {
  ...
  function FileInput() {
    if (!$(".upload-files-input-elem")[0]) {
      $(document.body).append(
        $('<input class="upload-files-input-elem" type="file" multiple />')
      );
    }
  }
  ...
  FileUploadDialog.prototype.show = function() {
    ...
    Dialogs.showModalDialogUsingTemplate(
      Mustache.render(dialogHTML, Strings),
      false
    );
    ...
    var $fromComputerButton = $dlg.find(
      ".dialog-button[data-button-id='from-computer']"
    );
    var $takeSelfieButton = $dlg.find(
      ".dialog-button[data-button-id='take-selfie']"
    );
  ...
  FileUploadDialog.prototype._handleTakeSelfie = function() {
    ...
    CommandManager.execute("bramble.selfie")
      .done(function(filename) {
        ...
        CommandManager.execute(CMD_OPEN, { fullPath: filename }).then(
          deferred.resolve,
          deferred.reject
        );
      })
