...
define(function(require, exports, module) {
  ...
  var BracketsFiler = brackets.getModule(
  ...
  function _handleClicksOnDirectories(pathToMove) {
    ...
    $(".move-to-dialog .directory-name")
      ...
      .click(function(e) {
        ...
        $directories.attr(
          "data-destination",
          $selectedDirectory.parent().attr("data-path")
        );
  ...
  function _getListOfDirectories(defaultPath, callback) {
    ...
    function constructDirTree(tree, currentNode, index) {
      ...
      if (currentNode.contents && currentNode.contents.length > 0) {
        ...
        directory.children = currentNode.contents.reduce(
          constructDirTree,
          false
        );
  ...
  function open() {
    ...
    _getListOfDirectories(defaultPath, function(err, directories) {
      ...
      var directoryContents = {
        directories: Mustache.render(
          directoryTreeTemplate,
          directories,
          subdirectoryContents
        )
      };

      var dialogHTML = Mustache.render(
        dialogTemplate,
        dialogContents,
        directoryContents
      );
