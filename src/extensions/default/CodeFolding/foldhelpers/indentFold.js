...
define(function(require, exports, module) {
  ...
  function indentFold(cm, start) {
    ...
    var lineIndent = cols(lineText, null, tabSize),
      collapsible = false,
    ...
    for (i = start.line + 1; i < lineCount; i++) {
      ...
      if (
        currentLine.trim().length !== 0 && (token && token.type !== "comment")
      ) {
        if (!collapsible) {
          ...
          if (indent <= lineIndent) {
            return {
              from: pos(start.line, lineText.length),
